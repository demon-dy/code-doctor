...
import type { CodeDoctorConfig, CodeGraph, GraphEdge, GraphNode } from "../types.js";
...
interface GraphCollector {
  nodes: Map<string, GraphNode>;
  edges: Map<string, GraphEdge>;
  functions: FunctionRecord[];
}

const walkSourceFiles = async (root: string): Promise<string[]> => {
  const files: string[] = [];
...
const addNode = (collector: GraphCollector, node: GraphNode): void => {
...
const endpointId = (method: string, route: string): string =>
  `endpoint:${method.toUpperCase()}:${normalizeRoute(route)}`;
...
const resolveImportTarget = async (
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (stat.isFile() && candidate.startsWith(root)) return relative(root, candidate);
    } catch {}
  }
  return undefined;
};
...
const methodFromOptions = (expression: ts.Expression | undefined): string => {
  ...
  for (const property of expression.properties) {
    if (!ts.isPropertyAssignment(property) || property.name.getText().replaceAll(/["']/g, "") !== "method") continue;
...
const analyzeTypeScript = async (
  ...
  const visit = (node: ts.Node, currentFunction?: FunctionRecord): void => {
    ...
    if (ts.isCallExpression(node)) {
      ...
      if (calledName && nextFunction) nextFunction.calls.push({ name: calledName, line });
...
const connectFunctionCalls = (collector: GraphCollector): void => {
...
export const analyzeCodeGraph = async (input: {
  ...
  const collector: GraphCollector = {
    nodes: new Map(),
    edges: new Map(),
    functions: [],
  };
