...
const projectAuditPrompt = (taskFile: string): string => `你是 Code Doctor 的全项目业务逻辑审计 Agent。读取任务文件：${taskFile}
...
const missingFileFinding = (scenario: ProjectReportScenario): ProjectAuditFinding | undefined => {
  if (!scenario.map || !scenario.missingEvidenceFiles.length) return undefined;
...
export const auditProjectBusiness = async (input: {
  ...
  for (const scenario of mapped) {
    ...
    if (missing) structural.push(missing);
  }
  const boundaries = [...project.unknownBoundaries];
  if (mapped.length < project.scenarios.length) boundaries.push(`${project.scenarios.length - mapped.length} 个候选场景尚未建图，AI 和结构审计均无法覆盖其内部业务逻辑。`);
  const stale = mapped.filter((scenario) => scenario.freshness === "stale");
  if (stale.length) boundaries.push(`${stale.length} 个场景地图已过期，本次风险结论可能遗漏最新代码变化。`);
  if (!mapped.length) boundaries.push("没有已建图场景；本次只能记录覆盖边界，不能判断业务链是否完整。");

  const base: Omit<ProjectAuditReport, "summary"> = {
    schemaVersion: 1,
  ...
  if (input.useAgent === false) base.boundaries.push("本次使用 --no-agent，仅完成确定性结构审计；不可达结果、条件覆盖和跨场景矛盾等业务语义风险尚未由 AI 复核。");
    ...
    try {
      ...
      if (agent.exitCode !== 0) throw new Error(`全项目业务审计 Agent 执行失败，退出码 ${agent.exitCode}`);
      let candidate: unknown;
      try {
        candidate = JSON.parse(await fs.readFile(candidateFile, "utf8")) as unknown;
      } catch (error) {
        if ((error as NodeJS.ErrnoException).code === "ENOENT") throw new Error("全项目业务审计 Agent 未按协议写入 candidateFile");
        throw new Error(`全项目业务审计 Agent 输出的 JSON 无效：${(error as Error).message}`);
      }
      ...
      for (const question of finalized.unansweredQuestions) base.boundaries.push(`AI 待确认：${question}`);
      await fs.rm(candidateFile, { force: true });
    } catch (error) {
  ...
  const audit: ProjectAuditReport = { ...base, summary: summarizeProjectAudit(base) };
