import type {
  ProjectAuditFinding,
  ProjectAuditNodeReference,
  ProjectReportScenario,
} from "../types.js";
import { stableId } from "../utils.js";

const nodeRefs = (scenarioId: string, nodeIds: string[]): ProjectAuditNodeReference[] =>
  [...new Set(nodeIds)].sort().map((nodeId) => ({ scenarioId, nodeId }));

const finding = (input: {
  scenario: ProjectReportScenario;
  code: string;
  category: ProjectAuditFinding["category"];
  severity: ProjectAuditFinding["severity"];
  title: string;
  conclusion: string;
  reasoning: string[];
  nodeIds?: string[];
...
const normalizeGuard = (value?: string): string => (value ?? "").trim().toLocaleLowerCase().replaceAll(/\s+/g, " ");

export const findStructuralRisks = (scenario: ProjectReportScenario): ProjectAuditFinding[] => {
  const map = scenario.map;
  if (!map) return [];
  const findings: ProjectAuditFinding[] = [];
  const byId = new Map(map.nodes.map((node) => [node.id, node]));
  const incoming = new Map(map.nodes.map((node) => [node.id, [] as typeof map.edges]));
  const outgoing = new Map(map.nodes.map((node) => [node.id, [] as typeof map.edges]));
  for (const edge of map.edges) {
    incoming.get(edge.to)?.push(edge);
    outgoing.get(edge.from)?.push(edge);
  }
  const entries = map.nodes.filter((node) => !(incoming.get(node.id)?.length));
  if (!entries.length) findings.push(finding({
  ...
  if (!outcomes.length) findings.push(finding({
  ...
  if (isolated.length) findings.push(finding({
  ...
  const deadEnds = map.nodes.filter((node) =>
    node.kind !== "outcome" && node.kind !== "side_effect" && !isolated.includes(node) && !(outgoing.get(node.id)?.length));
  if (deadEnds.length) findings.push(finding({
  ...
  for (const decision of map.nodes.filter((node) => node.kind === "decision")) {
    ...
    if (branches.length < 2) findings.push(finding({
    ...
    if (empty.length || duplicated.length) findings.push(finding({
  ...
  while (unvisited.size) {
    ...
    while (queue.length) {
      ...
      for (const neighbour of neighbours) if (unvisited.delete(neighbour)) queue.push(neighbour);
    }
    components.push(component.sort());
  }
  if (components.length > 1 && map.nodes.length > 1) findings.push(finding({
  ...
  if (duplicateEvidence.length || invalidReferences.length || noEvidenceNodes.length || noEvidenceEdges.length) findings.push(finding({
  ...
  if (conflictedNodes.length || conflictedEdges.length) findings.push(finding({
