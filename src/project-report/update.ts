...
import type {
  ...
  ProjectImpactChangeStatus,
  ProjectImpactChangedFile,
  ProjectImpactReport,
  ProjectImpactScenario,
} from "../types.js";
...
export const validateGitRange = (value: string): string => {
...
export const rangeFromBase = (value: string): string => `${validateGitRange(value)}...HEAD`;

const normalizeFile = (value: string): string => value.replaceAll("\\", "/").replace(/^\.\//, "");

const changeStatus = (value: string): ProjectImpactChangeStatus => {
  switch (value[0]) {
    case "A": return "added";
...
export const parseGitNameStatus = (stdout: string): GitChangedPath[] => {
  ...
  for (let index = 0; index < fields.length;) {
    ...
    if (status === "renamed" || status === "copied") {
      ...
      if (previousPath && currentPath) changed.push({ path: currentPath, previousPath, status });
    } else {
      const file = normalizeFile(fields[index++] ?? "");
      if (file) changed.push({ path: file, status });
    }
  }
  return changed.filter((item) => !item.path.startsWith(".code-doctor/"));
};

export const readGitChanges = async (root: string, unsafeRange: string): Promise<GitChangedPath[]> => {
...
const isBusinessSource = (file: string, config: CodeDoctorConfig): boolean => {
...
export const isGlobalProjectFile = (file: string): boolean => {
  const value = normalizeFile(file).toLowerCase();
  const basename = path.posix.basename(value);
  if (/^(package(-lock)?\.json|pnpm-lock\.yaml|yarn\.lock|go\.(mod|sum)|code-doctor\.ya?ml|tsconfig(?:\.[^.]+)?\.json)$/.test(basename)) return true;
  if (/^(vite|vue|next|nuxt|webpack|rollup|eslint|babel|jest|vitest)\.config\./.test(basename)) return true;
  return /(^|\/)(main|app|index|router|routes|store)\.(ts|tsx|js|jsx|mjs|cjs|vue|go)$/.test(value)
    || /(^|\/)\.env(?:\.|$)/.test(value);
};

const aliases = (change: GitChangedPath): string[] => [change.path, ...(change.previousPath ? [change.previousPath] : [])].map(normalizeFile);
...
export const analyzeProjectImpact = (input: {
  ...
  const changedFiles: ProjectImpactChangedFile[] = input.changes.map((change) => {
  ...
  if (unattributedSource.length) unknownBoundaries.push(`${unattributedSource.length} 个业务源码变更尚未归属任何场景，不能判断为“无业务影响”：${unattributedSource.map((file) => file.path).join("、")}`);
  const globalChanges = changedFiles.filter((file) => file.global);
  if (globalChanges.length) unknownBoundaries.push(`检测到 ${globalChanges.length} 个全局入口、依赖或配置变更，可能跨越当前证据关联：${globalChanges.map((file) => file.path).join("、")}`);
  const destructive = changedFiles.filter((file) => file.status === "deleted" || file.status === "renamed");
  if (destructive.length) unknownBoundaries.push(`${destructive.length} 个文件被删除或重命名；已同时使用旧路径和新路径做影响关联，仍需复核失效入口与证据。`);
  const unmapped = scenarios.filter((scenario) => !scenario.map);
  if (unmapped.length) unknownBoundaries.push(`${unmapped.length} 个已发现业务场景尚未建图，增量分析无法证明这些场景未受影响。`);
  return {
    changedFiles,
    impactedScenarios,
...
const readImpact = async (file: string): Promise<ProjectImpactReport | undefined> => {
  try {
    const value = JSON.parse(await fs.readFile(file, "utf8")) as ProjectImpactReport;
...
const writeImpact = async (file: string, report: ProjectImpactReport): Promise<void> => {
...
const sameChanges = (left: ProjectImpactChangedFile[], right: ProjectImpactChangedFile[]): boolean =>
  JSON.stringify(left.map(({ path: file, previousPath, status }) => ({ file, previousPath, status })))
  === JSON.stringify(right.map(({ path: file, previousPath, status }) => ({ file, previousPath, status })));

export const updateProjectFromGit = async (input: {
  root: string;
  config: CodeDoctorConfig;
  range: string;
  limit?: number;
  ...
  if (input.limit !== undefined && (!Number.isInteger(input.limit) || input.limit < 1)) throw new Error("--limit 必须是大于 0 的整数");
  ...
  const impactedScenarios: ProjectImpactScenario[] = analyzed.impactedScenarios.map((scenario) => {
  ...
  const impact: ProjectImpactReport = {
    schemaVersion: 1,
    createdAt,
  ...
  if (!useAgent) {
    for (const scenario of impact.impactedScenarios) if (scenario.status !== "completed") scenario.status = "skipped_no_agent";
    ...
    for (const scenario of impact.impactedScenarios) {
      if (scenario.status === "completed" || attempted >= (input.limit ?? Number.POSITIVE_INFINITY)) continue;
      attempted += 1;
  ...
  if (pending.length) impact.unknownBoundaries.push(`${pending.length} 个受影响场景尚未重新建图，本次影响结论不完整。`);
  if (failed.length) impact.unknownBoundaries.push(`${failed.length} 个受影响场景重建失败：${failed.map((scenario) => `${scenario.title}（${scenario.error ?? "未知错误"}）`).join("、")}`);
