...
const compareScenario = (left: KnowledgeAtlasEntry, right: KnowledgeAtlasEntry): number => {
...
export const orderProjectScenarios = (entries: KnowledgeAtlasEntry[]): KnowledgeAtlasEntry[] => {
  const byId = new Map(entries.map((entry) => [entry.id, entry]));
  const indegree = new Map(entries.map((entry) => [entry.id, 0]));
  const downstream = new Map(entries.map((entry) => [entry.id, [] as string[]]));
  for (const entry of entries) {
    for (const dependency of new Set(entry.dependsOn ?? [])) {
      if (dependency === entry.id || !byId.has(dependency)) continue;
  ...
  const ordered: KnowledgeAtlasEntry[] = [];
  while (ready.length) {
...
const readState = async (file: string): Promise<ProjectBuildRunState | undefined> => {
  try {
    const value = JSON.parse(await fs.readFile(file, "utf8")) as ProjectBuildRunState;
...
const writeState = async (file: string, state: ProjectBuildRunState): Promise<void> => {
...
const defaultDependencies: ProjectBuildDependencies = {
  list: listScenarios,
...
const summarizeStatus = (scenarios: ProjectBuildScenarioRun[]): ProjectBuildRunState["status"] => {
...
export const runAllProjectScenarios = async (input: {
  root: string;
  ...
  for (const entry of ordered) {
    if (!/^[a-z0-9][a-z0-9_-]*$/.test(entry.id)) throw new Error(`业务场景 id 无效：${entry.id}`);
    if (ids.has(entry.id)) throw new Error(`业务场景 id 重复：${entry.id}`);
    ids.add(entry.id);
  }
  const previous = await readState(stateFile);
  const previousById = new Map(previous?.scenarios.map((scenario) => [scenario.id, scenario]) ?? []);
  const scenarios: ProjectBuildScenarioRun[] = [];
  for (const entry of ordered) {
    const old = previousById.get(entry.id);
    const mapExists = await dependencies.hasMap(input.root, entry.id);
    let status: ProjectBuildScenarioRun["status"];
    if (input.refresh) status = "pending";
    // Agent 可能已经保存地图，但进程在下一次状态落盘前被终止。只要不是一次明确
    // 失败的尝试，已有地图就是可恢复的完成证据，避免再次消耗 Agent 调用。
    else if (mapExists && old?.status !== "failed") status = "completed";
    else if (old?.status === "failed") status = "failed"; // 未真正开始重试前保留失败原因。
    else status = "pending"; // running 表示上次被中断，本次恢复为待处理。
    scenarios.push({
      id: entry.id,
      title: entry.title,
  ...
  const state: ProjectBuildRunState = {
    schemaVersion: 1,
  ...
  for (const scenario of state.scenarios) {
    if ((scenario.status !== "pending" && scenario.status !== "failed") || attempted >= (input.limit ?? Number.POSITIVE_INFINITY)) continue;
    attempted += 1;
  ...
  state.updatedAt = dependencies.now();
  if (state.status === "completed" || state.status === "completed_with_failures") state.completedAt = dependencies.now();
  else state.completedAt = undefined;
  await writeState(stateFile, state);
