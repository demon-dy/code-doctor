...
export interface ProjectDoctorReport {
  ...
  languages: string[];
  knowledge: { discovered: number; mapped: number; reviewed: number };
  agent: { provider: AgentConfig["provider"]; available: boolean; resolvedProvider?: string };
}
...
const detectLanguages = async (root: string): Promise<string[]> => {
...
const checkAgent = async (agent: AgentConfig): Promise<{ available: boolean; resolvedProvider?: string; message: string; suggestion?: string }> => {
  ...
  if (agent.provider === "auto") {
    if (await commandExists("codex")) return { available: true, resolvedProvider: "codex", message: "auto 将使用 codex" };
    if (await commandExists("claude")) return { available: true, resolvedProvider: "claude", message: "auto 将使用 claude" };
  ...
  if (!agent.command?.trim()) return { available: false, message: "custom Agent 缺少 command", suggestion: "在 code-doctor.yaml 设置 agent.command，并使用 {promptFile} / {root} 占位符或环境变量" };
...
export const inspectProject = async (root: string, configOverride?: CodeDoctorConfig): Promise<ProjectDoctorReport> => {
  ...
  let config: CodeDoctorConfig | undefined;
  try {
    ...
    checks.push({ id: "config", status: "fail", message: `配置无法读取：${(error as Error).message}`, suggestion: `修复 ${CONFIG_FILE} 的 YAML 格式和字段` });
  }
  ...
  let mapped = 0;
  for (const entry of entries) if (await loadScenario(root, entry.id).then(() => true).catch(() => false)) mapped += 1;
