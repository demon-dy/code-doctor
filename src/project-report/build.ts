...
const buildScenario = async (
  ...
  buildRun?: ProjectBuildRunState["scenarios"][number],
...
export const buildProjectReport = async (root: string): Promise<{
  ...
  for (const scenario of scenarios) {
    ...
    if (missingDependencies.length) unknownBoundaries.push(`“${scenario.title}”引用了不存在的前置场景：${missingDependencies.join("、")}。`);
  }
  ...
  const visit = (id: string, trail: string[]): void => {
    if (visiting.has(id)) {
      for (const item of trail.slice(trail.indexOf(id))) cyclic.add(item);
      return;
    }
    if (visited.has(id)) return;
    visiting.add(id);
    for (const dependency of byId.get(id)?.dependsOn ?? []) if (byId.has(dependency)) visit(dependency, [...trail, dependency]);
    visiting.delete(id);
    visited.add(id);
  };
  for (const scenario of scenarios) visit(scenario.id, [scenario.id]);
  if (cyclic.size) unknownBoundaries.push(`业务场景依赖存在循环：${[...cyclic].sort().join("、")}；批量建图会使用稳定顺序兜底，但接管顺序需人工校准。`);
  ...
  if (buildFailed.length) unknownBoundaries.push(`${buildFailed.length} 个业务场景建图失败；失败不代表业务不存在，需要修复 Agent 或证据问题后继续。`);
  for (const scenario of buildFailed) unknownBoundaries.push(`“${scenario.title}”建图失败：${scenario.buildError ?? "未知错误"}`);
  if (buildPending.length) unknownBoundaries.push(`${buildPending.length} 个业务场景仍待处理，本报告不是全场景建图结果。`);
  ...
  const audit = auditValue as Partial<ProjectAuditReport> | undefined;
  if (audit?.schemaVersion === 1 && Array.isArray(audit.findings) && audit.agent && audit.summary && Array.isArray(audit.boundaries)) {
    report.audit = audit as ProjectAuditReport;
    if (audit.sourceCommit && head && audit.sourceCommit !== head) report.unknownBoundaries.push("项目风险审计生成后源码提交已变化，需要重新运行 project audit。");
