...
const confidence = (value: unknown, name: string): number => {
...
const uniqueIds = (values: Array<{ id: string }>, name: string): Set<string> => {
...
const validateLocation = async (root: string, evidence: BusinessEvidence): Promise<void> => {
  if (!evidence.location) {
    if (evidence.kind === "source" || evidence.kind === "test") throw new Error(`证据 ${evidence.id} 缺少源码位置`);
    return;
  }
  const relative = evidence.location.file.replaceAll("\\", "/");
  const absolute = path.resolve(root, relative);
  const escaped = path.relative(root, absolute).startsWith("..");
  if (escaped) throw new Error(`证据 ${evidence.id} 引用了项目之外的文件`);
  const stat = await fs.stat(absolute).catch(() => undefined);
  if (!stat?.isFile()) throw new Error(`证据 ${evidence.id} 引用的文件不存在：${relative}`);
  ...
  if (evidence.location.line !== undefined) {
    ...
    if (evidence.location.line > lineCount) throw new Error(`证据 ${evidence.id} 的行号超过文件长度`);
  }
...
export const finalizeBusinessMap = async (input: {
  ...
  if (!rawNodes.length) throw new Error("业务地图至少需要一个业务节点");
  if (rawNodes.length > input.config.maxNodes) throw new Error(`业务节点 ${rawNodes.length} 超过限制 ${input.config.maxNodes}`);
  if (rawEvidence.length > input.config.maxEvidence) throw new Error(`业务证据 ${rawEvidence.length} 超过限制 ${input.config.maxEvidence}`);

  const evidence: BusinessEvidence[] = rawEvidence.map((value, index) => {
    const item = object(value, `evidence[${index}]`);
    const kind = string(item.kind, `evidence[${index}].kind`);
    if (!evidenceKinds.has(kind)) throw new Error(`evidence[${index}].kind 无效`);
    ...
    return {
      ...
      kind: kind as BusinessEvidence["kind"],
  ...
  const nodes: BusinessNode[] = rawNodes.map((value, index) => {
    ...
    if (!nodeKinds.has(kind)) throw new Error(`nodes[${index}].kind 无效`);
    if (!statuses.has(status)) throw new Error(`nodes[${index}].status 无效`);
    const refs = strings(item.evidenceIds ?? [], `nodes[${index}].evidenceIds`);
    for (const id of refs) if (!evidenceIds.has(id)) throw new Error(`节点引用了不存在的证据：${id}`);
    if (status === "fact" && !refs.length) throw new Error(`事实节点 ${item.id ?? index} 必须引用证据`);
    ...
    return {
      ...
      kind: kind as BusinessNode["kind"],
      summary: string(item.summary, `nodes[${index}].summary`),
      status: status as BusinessNode["status"],
      evidenceIds: refs,
    };
  });
  ...
  const edges: BusinessEdge[] = rawEdges.map((value, index) => {
    ...
    if (!nodeIds.has(from) || !nodeIds.has(to)) throw new Error(`边 ${from} → ${to} 引用了不存在的节点`);
    if (!statuses.has(status)) throw new Error(`edges[${index}].status 无效`);
    const refs = strings(item.evidenceIds ?? [], `edges[${index}].evidenceIds`);
    for (const id of refs) if (!evidenceIds.has(id)) throw new Error(`边引用了不存在的证据：${id}`);
    if (status === "fact" && !refs.length) throw new Error(`事实路径 ${item.id ?? index} 必须引用证据`);
    if (status === "confirmed" && !refs.some((id) => evidenceById.get(id)?.kind === "human")) {
    ...
    return {
      ...
      status: status as BusinessEdge["status"],
...
export const finalizeAuditReport = (input: {
  ...
  if (!Array.isArray(rawFindings)) throw new Error("审计报告必须包含 findings 数组");
  ...
  const findings: BusinessFinding[] = rawFindings.map((value, index) => {
    ...
    if (!findingStatuses.has(status)) throw new Error(`findings[${index}].status 无效`);
    ...
    for (const id of relatedNodeIds) if (!nodeIds.has(id)) throw new Error(`结论引用了不存在的业务节点：${id}`);
    for (const id of relatedEvidenceIds) if (!evidenceIds.has(id)) throw new Error(`结论引用了不存在的证据：${id}`);
    ...
    return {
      ...
      status: status as BusinessFinding["status"],
      severity: severity as BusinessFinding["severity"],
