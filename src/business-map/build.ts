...
export const buildBusinessMap = async (input: {
  ...
  let raw: unknown;
  try {
    raw = JSON.parse(await fs.readFile(candidateFile, "utf8")) as unknown;
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === "ENOENT") throw new Error("业务地图 Agent 未按协议写入 candidateFile");
    throw new Error(`业务地图 Agent 输出的 JSON 无效：${(error as Error).message}`);
  }
