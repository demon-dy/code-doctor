...
const checkPrompt = (taskFile: string): string => `你是 Code Doctor 的 AI 业务代码审计 Agent。请读取任务文件：${taskFile}
...
export const checkBusinessRequirement = async (input: {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") throw new Error("业务审计 Agent 未按协议写入 candidateFile");
