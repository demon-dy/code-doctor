  ...
  .action(async (options: { root: string; json?: boolean }) => {
    ...
    if (options.json) console.log(JSON.stringify(report));
      ...
      for (const check of report.checks) {
        ...
        if (check.suggestion) console.log(pc.dim(`  建议：${check.suggestion}`));
      }
    }
...
rootOption(project.command("start").description("一键初始化、发现、分批建图、业务审计并生成唯一项目 HTML"))
  ...
  .action(async (options: { root: string; limit: number; agent?: string; audit: boolean; open?: boolean }) => {
    const root = resolveRoot(options.root);
    const config = await loadConfig(root);
    configureAgent(config, options.agent);
    ...
    for (const message of result.messages) console.log(pc.yellow(message));
    if (!options.open) console.log(pc.dim(`打开报告：code-doctor project open -C ${JSON.stringify(root)}`));
    else await openLocalFile(result.htmlFile);
  });
...
rootOption(project.command("build").description("生成项目业务总览；使用 --all 可串行建立全部候选场景地图"))
  ...
  .action(async (options: { root: string; all?: boolean; refresh?: boolean; limit?: number; agent?: string }) => {
    ...
    if (result.report.unknownBoundaries.length) console.log(pc.yellow(`${result.report.unknownBoundaries.length} 个未知或审计边界需要关注`));
  });
