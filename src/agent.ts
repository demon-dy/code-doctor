...
const buildPrompt = (taskFile: string): string => `你正在执行 Code Doctor 的单问题修复任务。
...
export const createRepairTask = async (input: {
  ...
  maxChangedLines: number;
  verificationCommands: string[];
}): Promise<{ task: RepairTask; taskFile: string; promptFile: string }> => {
...
export const runRepairAgent = async (input: {
  ...
  if (provider === "codex") {
    ...
    if (!input.config.command) throw new Error("自定义 Agent 必须配置 agent.command");
