...
export const assertCleanWorktree = async (root: string): Promise<void> => {
  const result = await runCommand({ command: "git status --porcelain", cwd: root });
  if (result.exitCode !== 0) throw new Error("当前目录不是可用的 Git 仓库");
  if (result.stdout.trim()) throw new Error("工作区存在未提交修改，Code Doctor 不会覆盖用户工作");
};

export const currentBranch = async (root: string): Promise<string> => {
  const result = await runCommand({ command: "git branch --show-current", cwd: root });
  if (result.exitCode !== 0 || !result.stdout.trim()) throw new Error("无法读取当前 Git 分支");
  return result.stdout.trim();
};

export const createDoctorBranch = async (
  root: string,
  ...
  if (result.exitCode !== 0) throw new Error(`创建分支失败：${result.stderr || result.stdout}`);
  return branch;
};

export const changedStats = async (root: string): Promise<{
  files: number;
  added: number;
  deleted: number;
}> => {
...
export const commitDoctorFix = async (input: {
  ...
  if (add.exitCode !== 0) throw new Error(`暂存修改失败：${add.stderr}`);
  const message = `fix: 修复 ${input.diagnostic.rule}`;
  const commit = await runCommand({
    command: `git commit -m ${quote(message)}`,
    cwd: input.root,
  });
  if (commit.exitCode !== 0) throw new Error(`创建提交失败：${commit.stderr || commit.stdout}`);
...
export const pushAndCreateMergeRequest = async (input: {
  ...
  if (push.exitCode !== 0) throw new Error(`推送分支失败：${push.stderr || push.stdout}`);
  ...
  if (mr.exitCode !== 0) throw new Error(`创建 MR 失败：${mr.stderr || mr.stdout}`);
  const url = `${mr.stdout}\n${mr.stderr}`.match(/https?:\/\/\S+\/merge_requests\/\d+/)?.[0];
  return url ?? mr.stdout.trim();
};

export const restoreBranch = async (root: string, branch: string): Promise<void> => {
