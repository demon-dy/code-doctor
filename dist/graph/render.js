...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box } body { margin:0; background:var(--bg); color:var(--text); font:14px/1.45 ui-monospace,SFMono-Regular,Menlo,monospace; overflow:hidden }
    ...
    .node.dim { opacity:.08 } .link { stroke:#42505e; stroke-opacity:.55 } .link.inferred { stroke-dasharray:5 4; stroke:#9a7b55 } .link.dim { opacity:.04 }
  </style>
</head>
<body>
  ...
  <script>
    ...
    svg.addEventListener('pointermove',event=>{if(!dragging)return;const rect=svg.getBoundingClientRect();dragging.x=event.clientX-rect.left;dragging.y=event.clientY-rect.top;dragging.vx=dragging.vy=0});
    ...
    function tick(){
      ...
      nodeEls.forEach((el,index)=>el.setAttribute('transform','translate('+nodes[index].x+' '+nodes[index].y+')'));requestAnimationFrame(tick)
    }
    function show(node){const related=edges.filter(edge=>edge.from===node.id||edge.to===node.id);detail.innerHTML='<div class="title">'+esc(node.label)+'</div><span class="badge">'+node.kind+'</span>'+(node.language?'<span class="badge">'+node.language+'</span>':'')+(node.location?'<div class="row"><div class="label">源码</div><div class="value">'+esc(node.location.file)+(node.location.line?':'+node.location.line:'')+'</div></div>':'')+'<div class="row"><div class="label">上下游 '+related.length+'</div>'+related.map(edge=>{const other=byId.get(edge.from===node.id?edge.to:edge.from);return '<div class="edge" data-id="'+esc(other.id)+'">'+(edge.from===node.id?'→ ':'← ')+esc(other.label)+' <small>'+edge.kind+(edge.confidence==='inferred'?' · inferred':'')+'</small></div>'}).join('')+'</div>';detail.querySelectorAll('[data-id]').forEach(el=>el.addEventListener('click',()=>show(byId.get(el.dataset.id))));}
    function esc(value){return String(value).replace(/[&<>"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[char]))}
    search.addEventListener('input',()=>{const value=search.value.toLowerCase();const matches=new Set(nodes.filter(node=>!value||node.label.toLowerCase().includes(value)||node.id.toLowerCase().includes(value)).map(node=>node.id));nodeEls.forEach((el,index)=>el.classList.toggle('dim',value&&!matches.has(nodes[index].id)));edgeEls.forEach((el,index)=>el.classList.toggle('dim',value&&!matches.has(edges[index].from)&&!matches.has(edges[index].to)))});
    tick();
  </script>
</body>
</html>`;
};
export const writeGraphArtifacts = async (root, graph) => {
