...
const prompt = (taskFile) => `你是 Code Doctor 的项目接管 Agent。读取任务文件：${taskFile}
...
export const discoverBusinessScenarios = async (input) => {
    ...
    if (agent.exitCode !== 0)
        throw new Error(`项目发现 Agent 执行失败，退出码 ${agent.exitCode}`);
    const raw = JSON.parse(await fs.readFile(candidateFile, "utf8"));
    const projectSummary = text(raw.projectSummary, "projectSummary");
    if (!Array.isArray(raw.scenarios) || raw.scenarios.length < 1 || raw.scenarios.length > 20)
        throw new Error("项目发现必须输出 1 到 20 个业务场景");
    const now = new Date().toISOString();
    const scenarios = [];
    for (const [index, value] of raw.scenarios.entries()) {
        if (!value || typeof value !== "object")
            throw new Error(`scenarios[${index}] 必须是对象`);
        const item = value;
        const id = safeId(item.id);
        const priority = text(item.priority, `scenarios[${index}].priority`);
        if (!new Set(["critical", "high", "normal"]).has(priority))
            throw new Error(`场景 ${id} 的 priority 无效`);
    ...
    if (ids.size !== scenarios.length)
        throw new Error("项目发现输出了重复的场景 id");
    for (const scenario of scenarios)
        scenario.dependsOn = scenario.dependsOn?.filter((id) => ids.has(id) && id !== scenario.id);
