...
export const captureBusinessSourceState = async (root) => {
...
export const assertBusinessSourceUnchanged = async (root, before) => {
    if (before === undefined)
        return;
    const after = await captureBusinessSourceState(root);
    if (after !== before)
        throw new Error("业务审计 Agent 修改了项目源码或 Git 状态，已拒绝本次产物；请人工恢复这些变更");
};
