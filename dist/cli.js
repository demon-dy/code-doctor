...
rootOption(map.command("discover").description("从整个项目发现新 Owner 应该逐步理解的候选业务场景"))
    ...
    for (const scenario of result.scenarios)
        console.log(`${scenario.priority === "critical" ? pc.red("●") : scenario.priority === "high" ? pc.yellow("●") : "○"} ${scenario.id}  ${scenario.title}`);
...
rootOption(map.command("build").description("让 Agent 从代码和技术图生成可追溯业务地图"))
    ...
    if (result.knowledge)
        console.log(pc.cyan(`候选场景已沉淀：${result.knowledge.id}（需运行 map confirm 后才成为人工确认知识）`));
});
...
rootOption(project.command("build").description("生成项目业务总览；使用 --all 可串行建立全部候选场景地图"))
    ...
    if (options.all) {
        ...
        if (result.state.status === "partial")
            console.log(pc.yellow(`仍有待处理场景；再次运行相同命令将从 ${result.stateFile} 继续`));
        if (result.failed) {
    ...
    if (options.refresh || options.limit !== undefined || options.agent)
        throw new Error("--refresh、--limit 和 --agent 需要与 --all 一起使用");
...
rootOption(project.command("update").description("按 Git 变更重建受影响业务场景、重跑项目审计并更新唯一总览"))
    ...
    if (options.changed && options.base)
        throw new Error("--changed 和 --base 不能同时使用");
    const root = resolveRoot(options.root);
    const config = await loadConfig(root);
    if (typeof options.agent === "string")
        configureAgent(config, options.agent);
    ...
    if (pending)
        console.log(pc.yellow(`${pending} 个受影响场景尚未重建；再次运行相同范围可继续`));
...
rootOption(rule.command("list").description("列出人工确认的业务规则"))
    ...
    if (!rules.length)
        console.log(pc.yellow("尚无人工确认业务规则"));
    for (const item of rules)
        console.log(`${pc.green("●")} ${item.id} [${item.scenarioId}] ${item.statement} ${pc.dim(`· ${item.confirmedBy}`)}`);
});
rootOption(takeover.command("start").description("启动或继续项目接管计划"))
    ...
    if (!state.scenarios.length)
        console.log(pc.yellow("请先用 map build 建立至少一个业务场景"));
});
rootOption(takeover.command("next").description("获取依赖顺序中的下一个待理解场景"))
    ...
    if (!record)
        console.log(pc.yellow(`该场景尚未建图，请运行 map build --focus ${JSON.stringify(entry?.focus ?? next.title)} --id ${next.id}`));
...
rootOption(takeover.command("progress").description("查看项目渐进式接管进度"))
    ...
    for (const item of progress.state.scenarios)
        console.log(`${item.status === "understood" ? pc.green("✓") : item.status === "in_progress" ? pc.yellow("→") : "○"} ${item.id}  ${item.title}`);
});
rootOption(program.command("audit").description("按 Git 变更或定时任务持续审计已沉淀的业务场景"))
    ...
    .option("--agent <provider>", "auto、codex、claude 或 custom")
    .action(async (options) => {
    if (Boolean(options.changed) === Boolean(options.deep))
        throw new Error("audit 必须且只能选择 --changed <range> 或 --deep");
    const root = resolveRoot(options.root);
    const config = await loadConfig(root);
    configureAgent(config, options.agent);
    ...
    for (const item of result.selectedScenarios)
        console.log(`${pc.green("●")} ${item.id}  ${item.title}`);
    if (result.uncoveredFiles.length)
        console.log(pc.yellow(`${result.uncoveredFiles.length} 个变更文件尚未被任何业务场景覆盖`));
    if (result.deferredScenarios.length)
        console.log(pc.yellow(`${result.deferredScenarios.length} 个受影响场景因本次运行上限延后审计`));
    for (const item of result.reports)
        console.log(pc.dim(`${item.scenarioId}: ${item.htmlFile}`));
});
...
rootOption(ci.command("install").description("写入 GitLab MR、Push、定时审计模板"))
    ...
    .action(async (options) => {
    if (!new Set(["mr", "push", "daily", "both", "all"]).has(options.mode))
        throw new Error("CI mode 必须是 mr、push、daily、both 或 all");
    const files = await installGitLabCi(resolveRoot(options.root), options.mode);
