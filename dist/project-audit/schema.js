...
export const finalizeProjectAgentAudit = (input) => {
    ...
    if (!Array.isArray(candidate.findings))
        throw new Error("项目业务审计候选必须包含 findings 数组");
    const scenarios = new Map(input.project.scenarios.filter((scenario) => scenario.map).map((scenario) => [scenario.id, scenario]));
    const findings = candidate.findings.map((value, index) => {
        const item = object(value, `findings[${index}]`);
        const category = string(item.category, `findings[${index}].category`);
        const severity = string(item.severity, `findings[${index}].severity`);
        let status = string(item.status, `findings[${index}].status`);
        if (!categories.has(category))
            throw new Error(`findings[${index}].category 不是允许的业务逻辑风险类别`);
        ...
        if (!relatedScenarioIds.length)
            throw new Error(`findings[${index}] 至少引用一个业务场景`);
        for (const scenarioId of relatedScenarioIds)
            if (!scenarios.has(scenarioId))
                throw new Error(`结论引用了不存在或尚未建图的业务场景：${scenarioId}`);
        if (!Array.isArray(item.relatedNodes))
            throw new Error(`findings[${index}].relatedNodes 必须是数组`);
        const relatedNodes = item.relatedNodes.map((value, referenceIndex) => {
            ...
            if (!relatedScenarioIds.includes(scenarioId))
                throw new Error(`节点引用的场景未列入 relatedScenarioIds：${scenarioId}`);
            const scenario = scenarios.get(scenarioId);
            if (!scenario || !scenario.map?.nodes.some((node) => node.id === nodeId))
                throw new Error(`结论引用了不存在的业务节点：${scenarioId}/${nodeId}`);
            return { scenarioId, nodeId };
        });
        if (!Array.isArray(item.evidence))
            throw new Error(`findings[${index}].evidence 必须是数组`);
        const evidence = item.evidence.map((value, referenceIndex) => {
            ...
            if (!relatedScenarioIds.includes(scenarioId))
                throw new Error(`证据引用的场景未列入 relatedScenarioIds：${scenarioId}`);
            const scenario = scenarios.get(scenarioId);
            if (!scenario || !scenario.map?.evidence.some((entry) => entry.id === evidenceId))
                throw new Error(`结论引用了不存在的业务证据：${scenarioId}/${evidenceId}`);
            return { scenarioId, evidenceId };
        });
        const confidence = Number(item.confidence);
        if (!Number.isFinite(confidence) || confidence < 0 || confidence > 1)
            throw new Error(`findings[${index}].confidence 必须是 0 到 1`);
...
export const summarizeProjectAudit = (report) => ({
