...
const normalizedChangedFiles = async (root, range) => {
    if (!/^[A-Za-z0-9._/@~^:{}+\-]+$/.test(range))
        throw new Error("Git range 包含不安全字符");
...
const evidenceFiles = (record) => new Set(record.map.evidence.flatMap((item) => item.location?.file ? [item.location.file.replaceAll("\\", "/")] : []));
const selectChangedScenarios = async (root, entries, changedFiles) => {
    const selected = [];
...
const writeExecution = async (root, execution) => {
